...
export default function Page() {
  ...
  const handleEdit = (p: Product) => { setEditingId(p.id); setEditForm({ ...p }) }
  const handleSave = () => {
  ...
  const handleAdd = () => {
    ...
    setProducts(prev => [...prev, { id, name: newProduct.name, category: newProduct.category, qty: parseInt(newProduct.qty) || 0, price: parseFloat(newProduct.price) || 0 }])
    setNewProduct({ name: '', category: '', qty: '', price: '' })
    setShowAdd(false)
  }

  const inputStyle = { border: '1px solid #ddd', borderRadius: 6, padding: '4px 8px', fontSize: '0.9rem', width: '100%', outline: 'none' }
      ...
      <div style={{ flex: 1, padding: '20px 80px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f0f0f0', borderRadius: 999, padding: '8px 16px' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2.5"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.35-4.35"/></svg>
          ...
          {(['Todos', 'Em estoque', 'Maior Estoque', 'Baixo Estoque'] as Filter[]).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ borderRadius: 999, padding: '8px 20px', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', border: 'none', background: filter === f ? '#222' : '#ddd', color: filter === f ? 'white' : '#444', transition: 'all 0.15s' }}>{f}</button>
          ))}
          <button onClick={() => setShowAdd(true)} style={{ marginLeft: 'auto', background: '#C8102E', color: 'white', border: 'none', borderRadius: 999, padding: '8px 20px', fontWeight: 700, cursor: 'pointer', fontSize: '0.9rem' }}>+ Adicionar</button>
        </div>
            ...
            <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 1fr 1fr 1.5fr', padding: '16px 20px', borderBottom: '1px solid #eee', alignItems: 'center' }}>
              {editingId === p.id ? (
                ...
                <>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => handleEdit(p)} style={{ background: '#f0f0f0', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>Editar ✏️</button>
                    <button onClick={() => handleDelete(p.id)} style={{ background: '#f0f0f0', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>Apagar 🗑️</button>
                  </div>
                </>
              )}
            </div>
        ...
        {showAdd && (
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: 'white', borderRadius: 20, padding: 32, width: 420, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
              ...
              {[{ label: 'Nome', key: 'name' }, { label: 'Categoria', key: 'category' }, { label: 'Quantidade', key: 'qty', type: 'number' }, { label: 'Preço (R$)', key: 'price', type: 'number' }].map(f => (
                <div key={f.key} style={{ marginBottom: 14 }}>
                  <label style={{ fontWeight: 600, fontSize: '0.9rem', display: 'block', marginBottom: 4 }}>{f.label}</label>
                  <input type={f.type || 'text'} value={(newProduct as any)[f.key]} onChange={e => setNewProduct(p => ({ ...p, [f.key]: e.target.value }))} style={{ ...inputStyle, padding: '10px 14px', borderRadius: 10 }} />
                </div>
              ))}
